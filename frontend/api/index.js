...
app.post('/api/auth/register', async (req, res) => {
  ...
  const newUser = { id: Date.now(), name, email, semester, password: hashedPassword };
  users.push(newUser);
...
app.post('/api/auth/login', async (req, res) => {
  ...
  const token = jwt.sign({ id: user.id, email: user.email, name: user.name, semester: user.semester }, JWT_SECRET, { expiresIn: '2h' });
  res.json({ token, user: { id: user.id, name: user.name, email: user.email, semester: user.semester } });
});
